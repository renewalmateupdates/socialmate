...
// Connect modals are only needed once someone reaches step 3 and picks a
// modal-type platform — most cold-load sessions never touch them. Loading
// them lazily keeps them off the bundle that has to download and parse
// before this page can paint anything on a first visit, which onboarding
// always is.
const BlueskyConnectModal  = dynamic(() => import('@/components/BlueskyConnectModal'))
const TelegramConnectModal = dynamic(() => import('@/components/TelegramConnectModal'))
const MastodonConnectModal = dynamic(() => import('@/components/MastodonConnectModal'))
...
export default function OnboardingInner({ initialReferralCode }: { initialReferralCode: string | null }) {
  ...
  // Read server-side (see page.tsx) and passed down as a prop rather than
  // read here from document.cookie after mount — a post-mount read meant this
  // banner popped in after first paint on every referred visit, which was
