  ...
  if (!isPro) {
    return (
      <div className="min-h-screen bg-theme flex flex-col items-center justify-center p-6 text-center gap-4">
        ...
        <h1 className="text-2xl font-black text-theme">Newsletter Agent</h1>
        <p className="text-muted text-sm max-w-md">Available on Pro and Agency plans.</p>
    ...
    <div className="min-h-screen bg-theme p-6 md:p-10 max-w-2xl mx-auto">
      <div className="mb-8">
        ...
        {settings.last_sent_at && (
          <p className="text-xs text-muted mt-1">Last ran: {new Date(settings.last_sent_at).toLocaleDateString()}</p>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="space-y-5">
          ...
          <div className="bg-surface border border-theme rounded-2xl p-5 flex items-center justify-between">
            <div>
              <p className="font-bold text-theme">Enable Newsletter Agent</p>
              <p className="text-xs text-muted mt-0.5">Runs every Sunday at 9am UTC</p>
            </div>
            <button
          ...
          <div className="bg-surface border border-theme rounded-2xl p-5">
            <p className="text-xs font-bold uppercase tracking-wide text-muted mb-3">Mode</p>
          ...
          {/* Subject prefix */}
          <div className="bg-surface border border-theme rounded-2xl p-5">
            <label className="block text-xs font-bold uppercase tracking-wide text-muted mb-1">Subject prefix <span className="font-normal normal-case">(optional)</span></label>
            <input
              type="text"
            ...
            />
            <p className="text-xs text-muted mt-1">Prepended to the AI-generated subject: "The Weekly Drop: ..."</p>
          </div>

          {/* Custom intro */}
          <div className="bg-surface border border-theme rounded-2xl p-5">
            <label className="block text-xs font-bold uppercase tracking-wide text-muted mb-1">Custom intro note <span className="font-normal normal-case">(optional)</span></label>
            <textarea
            ...
            <div className="bg-surface border border-theme rounded-2xl p-5">
              ...
              <div className="space-y-1 max-h-40 overflow-y-auto">
                ...
                {settings.subscriber_emails.length === 0 && (
                  <p className="text-xs text-muted text-center py-3">No subscribers yet</p>
                )}
              </div>
            </div>
          )}
          ...
          {sends.length > 0 && (
            <div className="bg-surface border border-theme rounded-2xl p-5">
              <p className="text-xs font-bold uppercase tracking-wide text-muted mb-3">History</p>
              <div className="space-y-2">
                {sends.map(s => (
                  <div key={s.id} className="flex items-center justify-between p-3 bg-theme border border-theme rounded-xl">
                    <div>
                      <p className="text-sm font-semibold text-theme truncate max-w-xs">{s.subject}</p>
                      <p className="text-xs text-muted mt-0.5">{new Date(s.sent_at).toLocaleDateString()} · {s.mode === 'auto' ? `${s.subscriber_count} sent` : 'Draft'}</p>
                    </div>
                  </div>
                ))}
              </div>
