...
export default function BulkScheduler() {
  ...
  const toggleGlobalPlatform = (id: string) => {
    setGlobalPlatforms(prev => prev.includes(id) ? prev.filter(p => p !== id) : [...prev, id])
  }
      ...
      <div className="ml-56 flex-1 p-8">
          ...
          <div className="flex items-end gap-4 flex-wrap">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button onClick={applyStartDate}
                  className="text-xs font-semibold px-3 py-2 border border-gray-200 rounded-xl hover:border-gray-400 transition-all whitespace-nowrap">
                  Auto-fill Dates
                </button>
              </div>
            </div>
            <button onClick={applyGlobalPlatforms}
              className="text-xs font-semibold px-4 py-2 bg-black text-white rounded-xl hover:opacity-80 transition-all self-end">
              Apply Platforms
            </button>
          </div>
        </div>
        ...
        <div className="space-y-3 mb-6">
          {posts.map((post, index) => {
            ...
            return (
              <div key={post.id}
                className={`bg-white border rounded-2xl overflow-hidden transition-all ${post.status === 'saved' ? 'border-green-200 bg-green-50/30' : post.status === 'error' ? 'border-red-200' : 'border-gray-100'}`}>
                <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-100">
                  ...
                  <button onClick={() => removePost(post.id)}
                    className="text-gray-300 hover:text-red-400 transition-all text-lg leading-none">×</button>
                </div>
                <div className="relative">
                  <textarea
                    placeholder={`Post ${index + 1} — write your caption here...`}
                    value={post.content}
                  ...
                  <div className="absolute bottom-2 right-3 flex items-center gap-2">
                    <div className="w-16 bg-gray-100 rounded-full h-1">
                      <div className={`h-1 rounded-full transition-all ${isOver ? 'bg-red-500' : pct > 80 ? 'bg-orange-400' : 'bg-gray-400'}`}
                        style={{ width: `${pct}%` }} />
                    </div>
        ...
        <div className="flex items-center justify-between">
          <button onClick={addPost}
            className="flex items-center gap-2 text-sm font-semibold px-4 py-2.5 border border-dashed border-gray-300 rounded-xl hover:border-gray-500 transition-all text-gray-500 hover:text-black">
            + Add Another Post
          </button>
