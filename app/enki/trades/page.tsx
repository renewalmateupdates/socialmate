        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5">{tk('app_enki_trades.page_title')}</p>
            </div>
          </div>
          ...
          <nav className="flex items-center gap-4 flex-wrap">
            ...
            >
              {tk('app_enki_trades.nav_export_csv')}
            </button>
          </nav>
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-gray-950 border border-amber-400/20 rounded-2xl p-5">
            <p className="text-xs font-bold text-amber-500 uppercase tracking-widest mb-1">{tk('app_enki_trades.stat_total_trades')}</p>
            <p className="text-2xl font-extrabold text-amber-400">
              {allTrades.length > 0 ? allTrades.length : '—'}
            </p>
            <p className="text-xs text-gray-500 mt-0.5">{openCount} {tk('app_enki_trades.filter_open').toLowerCase()}</p>
          </div>

          {/* Win rate */}
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">{tk('app_enki_trades.stat_win_rate')}</p>
            <p className={`text-2xl font-extrabold ${winRate !== null ? (winRate >= 50 ? 'text-green-400' : 'text-red-400') : 'text-gray-600'}`}>
              {winRate !== null ? `${winRate}%` : '—'}
            </p>
            <p className="text-xs text-gray-500 mt-0.5">
              {sellsWithPnl.length > 0 ? `${wins.length}/${sellsWithPnl.length} ${tk('app_enki_trades.filter_closed').toLowerCase()} sells` : tk('app_enki_trades.stat_no_closed_sells')}
            </p>
          </div>

          {/* Total P&L */}
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">{tk('app_enki_trades.stat_total_pnl')}</p>
            ...
            <p className="text-xs text-gray-500 mt-0.5">{tk('app_enki_trades.stat_paper_gains')}</p>
          </div>

          {/* Avg P&L % */}
          <div className="bg-gray-900 border border-gray-800 rounded-2xl p-5">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">{tk('app_enki_trades.stat_avg_pnl')}</p>
            ...
            <p className="text-xs text-gray-500 mt-0.5">{tk('app_enki_trades.stat_per_closed')}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 mb-6">
          {(['all', 'open', 'closed'] as const).map((f) => (
            ...
            >
              {f === 'all' ? tk('app_enki_trades.filter_all') : f === 'open' ? tk('app_enki_trades.filter_open') : tk('app_enki_trades.filter_closed')}
              {f === 'open' && openCount > 0 && (
          ...
          {total > 0 && (
            <span className="ml-auto text-xs text-gray-500">
              {total !== 1 ? tk('app_enki_trades.trade_count_plural').replace('{count}', String(total)) : tk('app_enki_trades.trade_count').replace('{count}', String(total))}
            </span>
          )}
        </div>
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden mb-6">
          {loading ? (
            <div className="py-16 text-center">
              ...
              <p className="text-xs text-gray-400">{tk('app_enki_trades.loading_trades')}</p>
            </div>
          ) : trades.length === 0 ? (
            <div className="py-16 text-center">
              <p className="text-sm text-gray-500 mb-1">{tk('app_enki_trades.empty_no_trades')}</p>
              <p className="text-xs text-gray-600">{tk('app_enki_trades.empty_scan_note')}</p>
            </div>
          ) : (
              ...
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-gray-800 bg-gray-900/80">
                    <th className="px-4 py-3 text-left font-bold text-gray-500 uppercase tracking-wider">{tk('app_enki_trades.col_symbol')}</th>
                    <th className="px-4 py-3 text-left font-bold text-gray-500 uppercase tracking-wider">{tk('app_enki_trades.col_side')}</th>
                    <th className="px-4 py-3 text-right font-bold text-gray-500 uppercase tracking-wider">{tk('app_enki_trades.col_qty')}</th>
                    <th className="px-4 py-3 text-right font-bold text-gray-500 uppercase tracking-wider">{tk('app_enki_trades.col_entry')}</th>
                    <th className="px-4 py-3 text-right font-bold text-gray-500 uppercase tracking-wider">{tk('app_enki_trades.col_exit')}</th>
                    <th className="px-4 py-3 text-right font-bold text-gray-500 uppercase tracking-wider">{tk('app_enki_trades.col_pnl')}</th>
                    <th className="px-4 py-3 text-center font-bold text-gray-500 uppercase tracking-wider">{tk('app_enki_trades.col_status')}</th>
                    <th className="px-4 py-3 text-right font-bold text-gray-500 uppercase tracking-wider">{tk('app_enki_trades.col_score')}</th>
                    <th className="px-4 py-3 text-left font-bold text-gray-500 uppercase tracking-wider">{tk('app_enki_trades.col_date')}</th>
                  </tr>
                </thead>
                <tbody>
                  {trades.map((t) => {
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between">
            ...
            >
              {tk('app_enki_trades.pagination_prev')}
            </button>
            <p className="text-xs text-gray-500">
              {tk('app_enki_trades.pagination_page_of').replace('{page}', String(page)).replace('{total}', String(totalPages))}
            </p>
            <button
              ...
              className="text-xs font-bold px-4 py-2 rounded-xl bg-gray-900 border border-gray-700 text-gray-400 hover:border-amber-500 hover:text-amber-400 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
            >
              {tk('app_enki_trades.pagination_next')}
            </button>
          </div>
        )}
        ...
        <p className="text-center text-xs text-gray-700 mt-8">
          {tk('app_enki_trades.footer_scan_note')}
        </p>
      </div>
    </div>
  )
