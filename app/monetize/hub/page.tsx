...
function MonetizeHubInner() {
  ...
  if (!isPro) {
    return (
      <div className="min-h-screen bg-theme flex flex-col items-center justify-center p-6 text-center gap-4">
        ...
        <h1 className="text-2xl font-black text-theme">{t('app_creator_hub.pro_gate_title')}</h1>
        <p className="text-muted text-sm max-w-md">{t('app_creator_hub.pro_gate_desc')}</p>
        ...
        <Link href="/monetize" className="text-xs text-muted hover:text-theme">{t('app_creator_hub.back_overview')}</Link>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-theme p-6 md:p-10 max-w-2xl mx-auto">
      <div className="mb-8">
        <Link href="/monetize" className="text-xs text-muted hover:text-theme mb-4 inline-block">{t('app_creator_hub.back_short')}</Link>
        <div className="flex items-center gap-3 mb-2">
          <DollarSign className="w-7 h-7" strokeWidth={1.75} />
          <h1 className="text-2xl font-black text-theme">{t('app_creator_hub.title')}</h1>
          <span className="text-xs bg-amber-100 text-amber-700 font-bold px-2 py-0.5 rounded-full">Pro+</span>
        </div>
        <p className="text-muted text-sm">{t('app_creator_hub.subtitle')}</p>
      </div>

      {loading ? (
        <div className="text-muted text-sm py-10 text-center">{t('app_creator_hub.loading')}</div>
      ) : (
        <div className="space-y-5">
          ...
          {settings.stripe_onboarding_complete && creatorUrl && (
            <div className="bg-surface border border-theme rounded-2xl p-5">
              <p className="text-xs font-bold uppercase tracking-wide text-muted mb-3">{t('app_creator_hub.share_page')}</p>
              <div className="flex items-center gap-2 bg-theme border border-theme rounded-xl px-3 py-2 mb-3">
                <span className="text-xs text-muted truncate flex-1">{creatorUrl}</span>
                <button
                ...
                <a href={creatorUrl} target="_blank" rel="noopener noreferrer"
                  className="text-xs font-bold text-muted hover:text-theme shrink-0">
              ...
              <div className="grid grid-cols-3 gap-2">
                {settings.tip_enabled && (
                  <button
                    ...
                    className="py-2 rounded-xl border border-theme bg-theme text-xs font-bold text-amber-500 hover:bg-amber-50 dark:hover:bg-amber-900/20 transition-all"
                  >
                ...
                {settings.subscription_enabled && (
                  <button
                    ...
                    className="py-2 rounded-xl border border-theme bg-theme text-xs font-bold text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 transition-all"
                  >
                ...
                <a
                  ...
                  className="py-2 rounded-xl border border-theme bg-theme text-xs font-bold text-muted hover:text-theme hover:border-amber-400/50 transition-all text-center"
                >
          ...
          {settings.stripe_onboarding_complete && earnings && (
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-surface border border-theme rounded-2xl p-4 text-center">
                ...
                <p className="text-xs text-muted mt-1">{t('app_creator_hub.active_subscribers')}</p>
              </div>
            </div>
          )}
          ...
          <div className="bg-surface border border-theme rounded-2xl p-5 space-y-3">
            <p className="text-xs font-bold uppercase tracking-wide text-muted">{t('app_creator_hub.your_creator_page')}</p>
            <div>
              <label className="block text-xs text-muted mb-1">{t('app_creator_hub.page_handle_label')}</label>
              <div className="flex items-center gap-1">
                <span className="text-sm text-muted">socialmate.studio/creator/</span>
                <input
                  value={settings.page_handle ?? ''}
                  onChange={e => setSettings(s => ({ ...s, page_handle: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g,'') }))}
                  placeholder="yourhandle"
                  className="flex-1 bg-theme border border-theme rounded-lg px-3 py-1.5 text-sm text-theme focus:outline-none focus:border-amber-400"
                />
              </div>
            </div>
            <div>
              <label className="block text-xs text-muted mb-1">{t('app_creator_hub.display_name_label')}</label>
              <input
            ...
            <div>
              <label className="block text-xs text-muted mb-1">{t('app_creator_hub.bio_label')}</label>
              <textarea
                value={settings.page_bio ?? ''}
                onChange={e => setSettings(s => ({ ...s, page_bio: e.target.value }))}
                placeholder="Tell your audience who you are..."
                rows={2}
                className="w-full bg-theme border border-theme rounded-xl px-4 py-2 text-sm text-theme focus:outline-none focus:border-amber-400 resize-none"
              />
            </div>
            <div>
              <label className="block text-xs text-muted mb-2">{t('app_creator_hub.header_color_label')}</label>
          ...
          <div className="bg-surface border border-theme rounded-2xl p-5 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-bold text-theme">{t('app_creator_hub.tip_jar_title')}</p>
                <p className="text-xs text-muted mt-0.5">{t('app_creator_hub.tip_jar_desc')}</p>
              </div>
              <button
          ...
          <div className="bg-surface border border-theme rounded-2xl p-5 space-y-3">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-bold text-theme">{t('app_creator_hub.fan_subs_title')}</p>
                <p className="text-xs text-muted mt-0.5">{t('app_creator_hub.fan_subs_desc')}</p>
              </div>
              <button
            ...
            {settings.subscription_enabled && (
              <div className="space-y-3 pt-2 border-t border-theme">
                ...
                <div>
                  <label className="block text-xs text-muted mb-1">{t('app_creator_hub.tier_name')}</label>
                  <input
                ...
                <div>
                  <label className="block text-xs text-muted mb-1">{t('app_creator_hub.tier_desc_label')}</label>
                  <textarea
          ...
          {settings.stripe_onboarding_complete && (
            <div className="bg-surface border border-theme rounded-2xl p-5 space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-bold text-theme">{t('app_creator_hub.exclusive_posts')}</p>
                  <p className="text-xs text-muted mt-0.5">{t('app_creator_hub.exclusive_posts_desc')}</p>
                </div>
                <button
              ...
              {showNewPost && (
                <div className="space-y-3 pt-3 border-t border-theme">
                  <div>
                    <label className="block text-xs text-muted mb-1">{t('app_creator_hub.post_title_label')}</label>
                    <input
                      value={newPost.title}
                  ...
                  <div>
                    <label className="block text-xs text-muted mb-1">{t('app_creator_hub.preview_label')}</label>
                    <textarea
                  ...
                  <div>
                    <label className="block text-xs text-muted mb-1">{t('app_creator_hub.full_content_label')}</label>
                    <textarea
                  ...
                  <div>
                    <label className="block text-xs text-muted mb-1">{t('app_creator_hub.unlock_price_label')}</label>
                    <div className="flex items-center gap-2">
                      ...
                      <input
                        ...
                        className="flex-1 bg-theme border border-theme rounded-xl px-4 py-2 text-sm text-theme focus:outline-none focus:border-amber-400"
                      />
                    </div>
                    <p className="text-xs text-muted mt-1">{t('app_creator_hub.unlock_price_note')}</p>
                  </div>
                  <button
                    onClick={createPost}
              ...
              {posts.length > 0 ? (
                <div className="space-y-2 pt-2 border-t border-theme">
                  {posts.map(post => (
                    <div key={post.id} className="flex items-start justify-between gap-3 bg-theme border border-theme rounded-xl p-3">
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-semibold text-theme truncate">{post.title}</p>
                        <p className="text-xs text-muted truncate">{post.preview}</p>
                ...
                <p className="text-xs text-muted text-center py-4">{t('app_creator_hub.no_exclusive_posts')}</p>
              )}
            </div>
          )}
          ...
          {earnings && earnings.tips.length > 0 && (
            <div>
              <p className="text-xs font-bold uppercase tracking-wide text-muted mb-3">{t('app_creator_hub.recent_tips')}</p>
              <div className="space-y-2">
                {earnings.tips.slice(0, 5).map((tip, i) => (
                  <div key={i} className="bg-surface border border-theme rounded-xl p-3 flex items-center justify-between">
                    <div>
                      <p className="text-sm font-semibold text-theme">{tip.supporter_name || 'Anonymous'}</p>
                      {tip.message && <p className="text-xs text-muted italic">"{tip.message}"</p>}
                    </div>
          ...
          {earnings && earnings.subscriptions.length > 0 && (
            <div>
              <p className="text-xs font-bold uppercase tracking-wide text-muted mb-3">{t('app_creator_hub.fan_subscribers')} ({earnings.active_subscribers} active)</p>
              <div className="space-y-2">
                {earnings.subscriptions.slice(0, 5).map(sub => (
                  <div key={sub.id} className="bg-surface border border-theme rounded-xl p-3 flex items-center justify-between">
                    <p className="text-sm text-theme">{sub.subscriber_name || 'Fan'}</p>
