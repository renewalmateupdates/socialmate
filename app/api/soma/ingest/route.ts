...
export async function POST(req: NextRequest) {
  try {
    ...
    const { week_label, raw_input } = body as { week_label: string; raw_input: string }
    ...
    const { data: ingestion, error: insertError } = await admin
      ...
      .insert({
        ...
        user_id: user.id,
        week_label: week_label || 'This week',
        raw_input,
        extracted_insights,
    ...
    if (insertError || !ingestion) {
      console.error('[SOMA Ingest] DB insert error:', insertError)
    ...
    console.error('[SOMA Ingest POST]', err)
