...
export default function Queue() {
  ...
  const handleReschedule = async (id: string, time: string) => {
    ...
    const { error } = await supabase.from('posts').update({ scheduled_at: new Date(time).toISOString() }).eq('id', id)
  ...
  const handleBulkReschedule = async () => {
    ...
    const updatedPosts = await supabase.from('posts').select('*').eq('user_id', user.id).eq('status', 'scheduled').order('scheduled_at', { ascending: true })
          ...
          {loading ? (
            ...
            <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
              <div className="flex items-center gap-3 px-5 py-3 border-b border-gray-100">
                <input type="checkbox" checked={selected.size === filtered.length && filtered.length > 0} onChange={toggleSelectAll} className="w-4 h-4 rounded accent-black cursor-pointer" />
              ...
              {filtered.map((post, i) => {
                ...
                return (
                  <div key={post.id} className={`flex items-center gap-3 px-5 py-4 hover:bg-gray-50 transition-all group ${i !== filtered.length - 1 ? 'border-b border-gray-50' : ''} ${isOverdue ? 'bg-red-50/30' : ''}`}>
                    <input type="checkbox" checked={selected.has(post.id)} onChange={() => toggleSelect(post.id)} className="w-4 h-4 rounded accent-black cursor-pointer flex-shrink-0" />
                    ...
                    <div className="w-36 flex-shrink-0">
                      {rescheduleId === post.id ? (
                        <div className="flex gap-1">
                          <input type="datetime-local" value={rescheduleTime} onChange={e => setRescheduleTime(e.target.value)} className="text-xs px-2 py-1 border border-gray-200 rounded-lg focus:outline-none w-full" />
                          ...
                          <button onClick={() => { setRescheduleId(null); setRescheduleTime('') }} className="text-xs px-2 py-1 border border-gray-200 rounded-lg hover:border-gray-400">×</button>
                        </div>
                      ) : (
                        <div>
                    ...
                    <div className="flex items-center gap-1 w-24 flex-shrink-0 opacity-0 group-hover:opacity-100 transition-all">
                      <Link href={`/compose?edit=${post.id}`} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-gray-100 text-gray-400 hover:text-black transition-all text-sm">✏️</Link>
                      <button onClick={() => { setRescheduleId(post.id); setRescheduleTime('') }} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-gray-100 text-gray-400 hover:text-black transition-all text-sm">📅</button>
                      <button onClick={() => handleMoveToDraft(post.id)} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-gray-100 text-gray-400 hover:text-black transition-all text-sm">📂</button>
                      <button onClick={() => handleDelete(post.id)} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-red-50 text-gray-300 hover:text-red-400 transition-all">×</button>
                    </div>
                  </div>
                )
              })}
