...
export default function Hashtags() {
  ...
  const parseHashtags = (raw: string): string[] => {
    return raw.split(/[\s,]+/).map(h => h.trim().replace(/^#*/, '#').toLowerCase()).filter(h => h.length > 1)
  }
  ...
  const filtered = collections.filter(c => {
    const matchSearch = c.name.toLowerCase().includes(search.toLowerCase()) || c.hashtags.some(h => h.toLowerCase().includes(search.toLowerCase()))
              ...
              {filtered.map(collection => (
                <div key={collection.id} className="bg-white border border-gray-100 rounded-2xl p-5 flex flex-col gap-3 hover:border-gray-300 transition-all group">
                  ...
                  <div className="flex items-center gap-2 pt-2 border-t border-gray-50">
                    <button onClick={() => handleCopy(collection)} className="flex-1 py-1.5 text-xs font-semibold bg-black text-white rounded-xl hover:opacity-80 transition-all">
                      📋 Copy All
                    </button>
                    <button onClick={() => openEdit(collection)} className="px-3 py-1.5 text-xs font-semibold border border-gray-200 rounded-xl hover:border-gray-400 transition-all">
                      Edit
                    </button>
                    <button onClick={() => handleDelete(collection.id)} className="px-3 py-1.5 text-xs font-semibold text-red-400 border border-red-100 rounded-xl hover:border-red-300 transition-all">
                      ×
                    </button>
                  </div>
                </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input type="text" value={formName} onChange={e => setFormName(e.target.value)} placeholder="e.g. Fitness, Tech News, Brand Tags"
              ...
              <div>
                ...
                <textarea value={formHashtags} onChange={e => setFormHashtags(e.target.value)} placeholder="#fitness #workout #gym #motivation #health" rows={5}
                ...
                <p className="text-xs text-gray-400 mt-1">{parseHashtags(formHashtags).length} hashtags · Separate with spaces or commas · # is added automatically</p>
              </div>
              {formHashtags.trim() && (
