...
export default function LinkInBio() {
  ...
  const handleSaveLink = async () => {
    ...
    if (editingLink) {
      ...
      const { data, error } = await supabase.from('bio_links').insert({
        user_id: user.id, title: formTitle.trim(), url, icon: formIcon,
        active: true, order: newOrder, clicks: 0,
      }).select().single()
  ...
  const handleDrop = async (targetId: string) => {
    ...
    for (const l of updated) {
      await supabase.from('bio_links').update({ order: l.order }).eq('id', l.id)
    }
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-gray-400 mt-0.5">
              {pageUsername ? `socialmate.app/${pageUsername}` : 'Set up your bio page'}
            </p>
          </div>
            ...
            {loading ? (
              ...
              <div className="space-y-4">
                ...
                {links.length === 0 ? (
                  <div className="bg-white border border-gray-100 rounded-2xl p-10 text-center">
                    ...
                    <button onClick={openAddLink}
                      className="bg-black text-white text-xs font-semibold px-4 py-2 rounded-xl hover:opacity-80 transition-all">
                      Add Your First Link →
                    </button>
                  </div>
                ) : (
                      ...
                      <div key={link.id} draggable
                        ...
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                          ...
                          <button onClick={() => openEditLink(link)}
                            className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-gray-100 text-gray-400 hover:text-black transition-all text-sm">✏️</button>
              ...
              <div className="space-y-5 bg-white border border-gray-100 rounded-2xl p-6">
                ...
                <div>
                  ...
                  <textarea value={pageBio} onChange={e => setPageBio(e.target.value)}
              ...
              <div className={`rounded-3xl p-5 ${currentTheme.bg} border border-gray-200 shadow-xl min-h-96`}>
                ...
                <div className="space-y-2">
                  {activeLinks.slice(0, 5).map(link => (
                    <div key={link.id}
                      className={`w-full py-2.5 px-4 rounded-xl text-xs font-semibold text-center flex items-center justify-center gap-2 ${currentTheme.button}`}>
                      <span>{link.icon}</span>{link.title}
                    </div>
                  ))}
                  {activeLinks.length === 0 && (
                    <div className={`w-full py-2.5 px-4 rounded-xl text-xs font-semibold text-center opacity-30 ${currentTheme.button}`}>
                      Your Link Here
                    </div>
                  )}
                </div>
                {activeLinks.length > 5 && (
                  <p className={`text-xs text-center mt-2 opacity-50 ${currentTheme.text}`}>+{activeLinks.length - 5} more</p>
                )}
              </div>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <input type="text" value={formTitle} onChange={e => setFormTitle(e.target.value)}
                  placeholder="e.g. My Instagram, Shop Now, Portfolio"
                  className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none focus:border-gray-400" />
              </div>
              <div>
