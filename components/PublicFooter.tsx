...
// Single source of truth for the public footer. Rendered by PublicLayout (which
// wraps most public pages), by the home page, and by the handful of pages that
// import it directly — so every surface shows the identical instrument-styled
// footer. Warm tokens only (bg-void / border-edge / text-ink-*), never gray-*.
...
// className defaults to mt-16 (the spacing the old footers carried). The home
// page passes "" because its final Section already sets the vertical rhythm.
export default function PublicFooter({ className = 'mt-16' }: { className?: string }) {
  return (
    <footer className={`border-t border-edge bg-void ${className}`}>
      <div className="mx-auto w-full max-w-6xl px-gutter pt-12 pb-8">

        {/* Top: logo + socials */}
        <div className="mb-10 flex flex-col items-start justify-between gap-5 sm:flex-row sm:items-center">
          <div className="flex items-center gap-3">
            <img src="/logo.png" alt="SocialMate" className="h-8 w-8 rounded-lg" />
          ...
          <div className="flex items-center gap-1">
            {SOCIALS.map(s => (
              <a
                ...
                aria-label={s.label}
                className="flex h-8 w-8 items-center justify-center rounded-lg text-ink-faint transition-colors hover:bg-panel hover:text-ink-high"
              >
                {s.icon}
              </a>
            ))}
        ...
        <div className="grid grid-cols-2 gap-8 sm:grid-cols-4">
          {FOOTER_COLUMNS.map(col => (
            <div key={col.heading}>
              <p className="font-mono text-eyebrow uppercase text-ink-faint">{col.heading}</p>
        ...
        {/* Bottom: Product Hunt badge + copyright */}
        <div className="mt-12 flex flex-wrap items-center justify-between gap-4 border-t border-edge pt-8">
          <a
            href="https://www.producthunt.com/posts/socialmate-2"
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2 rounded-lg bg-[#FF6154] px-3 py-1.5 text-xs font-semibold text-white transition-colors hover:bg-[#e5564a]"
          >
